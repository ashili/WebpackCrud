...
city.create = function (newCity, result) {
  dbConn.query(
    'INSERT INTO tblCitiesImport set ?',
    newCity,
    function (err, res) {
      if (err) {
...
city.findById = function (id, result) {
  dbConn.query(
    'Select * from tblCitiesImport where id = ? ',
    id,
    function (err, res) {
      if (err) {
        console.log('error: ', err);
...
city.update = function (id, city, result) {
  dbConn.query(
    'UPDATE tblCitiesImport SET fldName=?,fldLat=?,fldLong=?,fldCountry=?,fldAbbreviation=?,fldCapitalStatus=?,fldPopulation=? WHERE id = ?',
    [
      city.fldName,
      city.fldLat,
      city.fldLong,
      city.fldCountry,
      city.fldAbbreviation,
      city.fldCapitalStatus,
      city.fldPopulation,
      id,
    ],
    function (err, res) {
...
city.delete = function (id, result) {
  dbConn.query(
    'DELETE FROM tblCitiesImport WHERE id = ?',
    [id],
    function (err, res) {
      if (err) {
        console.log('error: ', err);
